...
- **Duplicate detection**: enforced by a uniqueness constraint on the session identifier at the moment of ingest — a duplicate is rejected before it ever enters the queue, not deduplicated after the fact. Separately, if the application-reference field repeats under a *different* session identifier, that's neither silently accepted nor silently rejected — it's flagged into a distinct reviewable state, since the third party's field semantics for these two identifiers aren't documented anywhere reliable. See ADR-0003.
- **Retry is per-stage and resumable, never a full pipeline restart**: a form already transformed and persisted, but still owing its email, is retried by resending the email alone — never re-geocoded, never re-persisted, never given a second form record. The transformed-forms table itself carries its own uniqueness so this holds even under a retry bug, not only by convention.
...
- **Guaranteed email**: implemented as a transactional-outbox-style obligation — the email-pending state is written in the *same durable step* as the transformed record (never a separate later step that could be skipped), and delivery is retried until it succeeds rather than attempted once and forgotten. Exhausting one retry attempt or sweep cycle never permanently abandons the obligation.
- **Concurrency safety**: the ingest uniqueness check is enforced at the database level, not a check-then-insert race, and is covered by a concurrent-duplicate test. The recovery sweep claims rows before processing them so two overlapping sweep passes can't process the same row twice; on process startup, anything left mid-processing from a crash is picked back up.
...
- Three ADRs in `docs/adr/` record the architecture decisions this spec builds on: `0001-encrypted-sqlite-over-postgres.md`, `0002-db-backed-queue-not-external-broker.md`, and `0003-duplicate-identity-strategy.md`.
